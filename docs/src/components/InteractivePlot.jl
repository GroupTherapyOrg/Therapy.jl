...
@island function InteractivePlot(; frequency::Int = 5)
    ...
    create_effect(() -> begin
        ...
        js("if (!el) return")
        js("var f = \$1", freq())
        js("if (typeof Plotly === 'undefined') { var s = document.createElement('script'); s.src = 'https://cdn.plot.ly/plotly-2.35.2.min.js'; s.onload = function() { window._therapyPlotF = f; window._therapyPlotEl = el; var x=[],y=[]; for(var i=0;i<100;i++){x.push(i*0.1);y.push(Math.sin(i*0.1*f))} Plotly.newPlot(el,[{x:x,y:y,type:'scatter',mode:'lines',line:{color:'#389826',width:2}}],{margin:{t:20,r:20,b:40,l:40},paper_bgcolor:'transparent',plot_bgcolor:'transparent',xaxis:{gridcolor:'#e8e3d9',title:'x'},yaxis:{gridcolor:'#e8e3d9',title:'sin(x)',range:[-1.2,1.2]},font:{family:'JuliaMono,monospace',color:'#6b6560'}},{responsive:true,displayModeBar:false}) }; document.head.appendChild(s); return }")
        js("var x=[],y=[]; for(var i=0;i<100;i++){x.push(i*0.1);y.push(Math.sin(i*0.1*f))} Plotly.react(el,[{x:x,y:y,type:'scatter',mode:'lines',line:{color:'#389826',width:2}}],{margin:{t:20,r:20,b:40,l:40},paper_bgcolor:'transparent',plot_bgcolor:'transparent',xaxis:{gridcolor:'#e8e3d9',title:'x'},yaxis:{gridcolor:'#e8e3d9',title:'sin(x)',range:[-1.2,1.2]},font:{family:'JuliaMono,monospace',color:'#6b6560'}},{responsive:true,displayModeBar:false})")
    end)
