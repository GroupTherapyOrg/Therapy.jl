...
ResourceView(user;
    ...
    children = data -> UserCard(user = data)
        ...
        Section(:class => "py-12",
            ...
            Suite.Tabs(default_value="manual",
                Suite.TabsList(
                    Suite.TabsTrigger("Manual", value="manual"),
                    Suite.TabsTrigger("Pull-to-Refresh", value="pull"),
                    Suite.TabsTrigger("Auto-Refresh", value="auto")
                ),
                Suite.TabsContent(value="manual",
                    Suite.Card(class="mt-4",
...
delete_post = (id) -> begin
    ...
    refetch!(posts)  # Refresh list after deletion
            ...
            Suite.Alert(class="mt-8",
                Suite.AlertTitle("Automatic Invalidation"),
...
# Total: 250ms""",
            ...
            Suite.Alert(class="mt-8", variant="destructive",
                Suite.AlertTitle("Avoid Artificial Waterfalls"),
                Suite.AlertDescription(
        ...
        Section(:class => "py-12",
            ...
            # Show Suite.Skeleton in action
            Suite.Card(class="mb-6",
                Suite.CardHeader(Suite.CardTitle(:class => "font-serif", "Suite.Skeleton Demo")),
                Suite.CardContent(
                    Div(:class => "flex gap-4 p-4",
                        Suite.Skeleton(class="h-12 w-12 rounded-full"),
                        Div(:class => "flex-1 space-y-2",
                            Suite.Skeleton(class="h-4 w-1/3"),
                            Suite.Skeleton(class="h-3 w-1/2")
                        )
                    )
                )
            ),
...
function UserList(; users)
    For(() -> users) do user
        ...
        Div(:data_user_id => user.id,
            ...
            Suite.Alert(class="mt-8",
                Suite.AlertTitle("Prefetch Sparingly"),
                Suite.AlertDescription(
