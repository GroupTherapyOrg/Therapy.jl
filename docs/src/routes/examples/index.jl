() -> begin
    ...
    PageWithTOC(sections, Div(:class => "space-y-12",
        ...
        # ── Plot Gallery (WasmMakie — build-time html_snippet, zero wasm) ──
        Div(:class => "space-y-4",
            H2(:id => "plot-gallery", :class => "text-xl font-semibold text-warm-800 dark:text-warm-200", "Plot Gallery"),
            P(:class => "text-sm text-warm-600 dark:text-warm-400",
                "WasmMakie plots rendered at ", Strong("build time"), " through the embedding contract — each cell is ",
                Code(:class => "font-mono text-accent-500", "html_snippet(fig)"),
                ": a canvas, the recorded Canvas2D command stream, and the bundled replayer. No wasm, no hydration — they draw instantly."),
            Div(:class => "py-2", PlotGallery()),
        ),
