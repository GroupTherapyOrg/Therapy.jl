() -> begin
    Div(:class => "space-y-8",
        ...
        Pre(:class => "bg-warm-900 dark:bg-warm-950 p-4 rounded", Code("using Pkg\nPkg.add(\"Therapy\")")),
        
        H2(:class => "text-xl font-semibold text-warm-800 dark:text-warm-200", "Your First Component"),
        P(:class => "text-warm-500 dark:text-warm-400", "Components are plain Julia functions that return HTML elements:"),
        Pre(:class => "bg-warm-900 dark:bg-warm-950 p-4 rounded", Code("""function Greeting(; name="World")
    P("Hello, ", name, "!")
end""")),
        ...
        Pre(:class => "bg-warm-900 dark:bg-warm-950 p-4 rounded", Code("""@island function Counter(; initial::Int32 = Int32(0))
