...
test.describe('SPA Navigation - Multiple Clicks', () => {
  ...
  test('rapid clicking should not cause race conditions or duplicate fetches', async ({ page }) => {
    ...
    // Each unique WASM should be fetched at most ~3x during rapid clicks
    // (race between click handler and hydration guard can cause 1-2 extra fetches)
    // The old broken behavior was 60x duplicates; this catches regressions
    expect(wasmUrls.length).toBeLessThanOrEqual(uniqueWasmUrls.length * 3);
  });
});
